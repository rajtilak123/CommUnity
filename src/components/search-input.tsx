...
export function SearchInput({ className, containerClassName, ...props }: SearchInputProps) {
  return (
    <div className={cn("group relative w-full min-w-0", containerClassName)}>
      <Search className="pointer-events-none absolute left-0 top-1/2 size-4 -translate-y-1/2 text-[#A3A3A3] transition-colors group-focus-within:text-[#111111]" />
      <Input className={cn("pl-6 w-full min-w-0", className)} type="search" {...props} />
    </div>
  );
}
